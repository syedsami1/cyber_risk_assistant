...
# ✅ FIX: use environment variable (Render safe)
api_key = os.getenv("GROQ_API_KEY")

if not api_key:
    st.sidebar.error("GROQ_API_KEY not set in environment variables")

# Use relative paths from the project root
BASE_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../'))
data_dir = os.path.join(BASE_DIR, 'data')
ref_dir = os.path.join(BASE_DIR, 'data')
...
if not api_key:
    st.warning("Please configure GROQ_API_KEY in Render environment variables.")
    st.stop()
...
try:
    ...
    with st.spinner("Analyzing data..."):
        ...
        groq_assistant = GroqRiskAssistant(api_key=api_key)
